using System;
using System.Collections.Generic;

namespace tp_integrador_poo
{
	class Program
	{
		
		//------------------------------ VALIDACIONES ---------------------------------
		
		//validar y darle formato a saldo.
		static double ValidarSaldo(string saldo){
			double saldoValor = 0;
			//que no este vacio
			if(saldo == null || saldo.Trim() == "" ){
	    		throw new FormatException("Por favor ingrese un valor válido.");
	    	}
			
			//revisar el formato, que permita un punto o una coma como maximo
			int comas = 0;
			foreach(char l in saldo){//recorro lo que entro por consola
				if(l == '.' || l == ','){
					comas++;
					if(comas > 1){//si agregaron mas de una coma o punto mostrar error
						throw new FormatException("Solo se permite una coma o punto.");
					}
				}
			}
			//reemplazo el punto por coma, para que no me de error y haga bien las operaciones
			saldo = saldo.Replace('.',',');
			
			//convertir a double
			if(! double.TryParse(saldo, out saldoValor)){
        		throw new FormatException("Ingresar número válido.");
        	}
        	if(saldoValor < 0){
        		throw new ArgumentOutOfRangeException("Ingresar valor mayor a cero.");
        	}
			return saldoValor;
		}
		
		//validar número de cuenta
		static int ValidarNroCta(string nro){
			int nroValor = 0;
			if(nro == null || nro.Trim() == "" ){
	    		throw new FormatException("Por favor ingrese un valor válido.");
	    	}
			//revisar el formato, que solo sean numeros
			foreach(char no in nro){//recorro lo que entro por consola
				if(no < '0' || no > '9'){
					throw new FormatException("Solo se permiten números.");
				}
			}
			//convertir a double
			if(! int.TryParse(nro, out nroValor)){
        		throw new FormatException("Ingresar número válido.");
        	}
        	if(nroValor < 0){
        		throw new ArgumentOutOfRangeException("Ingresar valor mayor a cero.");
        	}
			return nroValor;
		}
		
		//buscamos cliente por dni
		static Cliente BuscarClientePorDNI(Banco banco, string dni){
			//traer listado de clientes
			Cliente cliente = null;
			
			//recuperar listado de clientes para recorrer
			List<Cliente> clientes = banco.ListarClientes();
			
			if(dni == null || dni.Trim() == "" ){
	    		throw new FormatException("Por favor ingrese un valor.");
	    	}else if(!(dni.Length >= 7 && dni.Length <= 8)){
    			throw new ArgumentOutOfRangeException("Ingrese entre 7 y 8 dígitos para el DNI. Por favor, ingrese un valor válido.");
    		}
	    	foreach(Cliente c in clientes){
	    		if(c.Dni == dni){
	    			//si el dni del objeto cliente es igual al dni ingresado
	    			return c;
	    		}
	    	}
			return null;
		}
		
		
		//-------------------------- CREAR CUENTAS --------------------------------------
		
		static void AgregarCuenta(Banco banco)
        {
            Console.WriteLine("=== AGREGAR CUENTA ===");
            
            string dni = "";
            Cliente cliente = null;
            bool dniValido = false;
            do{
            	Console.Write("DNI del cliente: ");
            	dni = Console.ReadLine();
            	try{
            		//buscar cliente pasarlo al main
	            	cliente = BuscarClientePorDNI(banco, dni);
	            	dniValido = true;
	            }catch(FormatException ex){
	            	Console.WriteLine(ex.Message);
	            }catch(IndexOutOfRangeException ex){
	            	Console.WriteLine(ex.Message);
            	}catch(ArgumentOutOfRangeException ex){
            		Console.WriteLine(ex.Message);
	            }
            }while(!dniValido);
            
			//si no existe el cliente, se crea uno nuevo
            if (cliente == null)
	        {
        		cliente = new Cliente();
        		//pido los datos del cliente
        		
        		cliente.Dni = dni; //agrego el dni agregado al inicio para verificar si existe
        		
                string nombre = "";
        		bool nombreValido = false;
        		do{
        			Console.Write("Nombre: ");
                	nombre = Console.ReadLine();
	            	try{
        				cliente.Nombre = nombre; //propiedad nombre para que afecte en el set
		            	nombreValido = true;
		            }catch(FormatException ex){
		            	Console.WriteLine(ex.Message);
		            }
	            }while(!nombreValido);
        		
                string apellido = "";
                bool apellidoValido = false;
        		do{
                	Console.Write("Apellido: ");
                	apellido = Console.ReadLine();
	            	try{
        				cliente.Apellido = apellido;
		            	apellidoValido = true;
		            }catch(FormatException ex){
		            	Console.WriteLine(ex.Message);
		            }
	            }while(!apellidoValido);
                
                string direccion = "";
                bool direccionValido = false;
        		do{
                	Console.Write("Dirección: ");
                	direccion = Console.ReadLine();
	            	try{
        				cliente.Direccion = direccion;
		            	direccionValido = true;
		            }catch(FormatException ex){
		            	Console.WriteLine(ex.Message);
		            }
	            }while(!direccionValido);
                
                string telefono = "";
                bool telefonoValido = false;
        		do{
                	Console.Write("Teléfono: ");
                	telefono = Console.ReadLine();
	            	try{
                		cliente.Telefono = telefono;
		            	telefonoValido = true;
		            }catch(FormatException ex){
		            	Console.WriteLine(ex.Message);
		            }
	            }while(!telefonoValido);
                
                string mail = "";
                bool mailValido = false;
        		do{
                	Console.Write("Mail: ");
                	mail = Console.ReadLine();
	            	try{
                		//solo valido que no este vacio
        				cliente.Mail = mail;
		            	mailValido = true;
		            }catch(FormatException ex){
		            	Console.WriteLine(ex.Message);
		            }
	            }while(!mailValido);

                //agrego cliente
                banco.AgregarCliente(cliente);
            }
            
            double saldoValor = 0;
            bool saldoValido = false;
            do{
            	Console.Write("Saldo inicial: ");
            	string saldo = Console.ReadLine();
	            try{
            		saldoValor = ValidarSaldo(saldo);
	            	saldoValido = true;
	            }catch(FormatException ex){
	            	Console.WriteLine(ex.Message);
	            }catch(ArgumentOutOfRangeException ex){
	            	Console.WriteLine(ex.Message);
	            }catch(Exception ex){
	            	Console.WriteLine(ex.Message);
	            }
            }while(!saldoValido);
            
            int nroCuenta = banco.CantidadCuentas() + 1;
            Cuenta cuenta = new Cuenta(nroCuenta, dni, cliente.Apellido, saldoValor);
            banco.AgregarCuenta(cuenta);

            Console.WriteLine("La cuenta fue creada con éxito.");
        }
		
		
		//---------------------------- BUSCAR CUENTA ------------------------------------
	    static Cuenta BuscarCuenta(Banco banco, int nroCuenta)
	    {
	    	if(banco.CantidadCuentas() > 0){
		        //aca hay que recorrer la lista con un foreach
		        foreach(Cuenta c in banco.ListarCuentas()){
		    		if(c.NroCuenta == nroCuenta){
		    			return c;
		    		}
		    	}
	    		return null;
	    	}else{
	    		return null;
	    	}
	    }

		
		//------------------------- ELIMINAR CUENTAS -------------------------------------
		
        static void EliminarCuenta(Banco banco)
        {
            Console.WriteLine("=== ELIMINAR CUENTA ===");
            
            //aca mostrar un listado de todas las cuentas que existen para poder seleccionar una
            
            //validar dato ingresado que sea numerico
            bool nroValido = false;
            int nroCuenta = 0;
            string nro = Console.ReadLine();
            do{
            	Console.Write("Ingrese número de cuenta: ");
            	try{
            		if(nro == null || nro.Trim() == "" ){
			    		throw new FormatException("Por favor ingrese un valor. Ingrese nuevamente.");
			    	}
            		foreach(char v in nro){
			    		if(!(v >= '0' && v <= '9')){
			    			throw new FormatException("Solo se permiten números. Ingrese nuevamente.");
			    		}
			    	}
            		nroCuenta = int.Parse(nro);
					nroValido = true;   
            	}catch(FormatException ex){
            		Console.WriteLine(ex.Message);
            	}catch(Exception ex){
            		Console.WriteLine(ex.Message);
            	}
            }while(!nroValido);
            
            Cuenta cuenta = BuscarCuenta(banco, nroCuenta);

            if (cuenta != null)
            {
            	//recupero el dni del titular de la cuenta, para buscar mas cuentas del cliente
            	string dniTitular = cuenta.DniTitular;

            	//elimino la cuenta del listado de cuentas
            	banco.EliminarCuenta(cuenta);
                Console.WriteLine("Cuenta eliminada con éxito.");
                
                //buscamos si el cliente tiene mas cuentas con la recursiva
                //si el resultado es 0 se elimina el cliente
                if(ContarCuentasCliente(banco, dniTitular) > 0){
                	//buscar cliente
                	Cliente cliente = BuscarClientePorDNI(banco, dniTitular);
                	//eliminar cliente
                	banco.EliminarCliente(cliente);
                	Console.WriteLine("El Cliente fue eliminado, no tiene mas cuentas en este banco.");
                }
            }
            else
            {
                Console.WriteLine("No se encontró la cuenta.");
            }
        }
        
        
        //------------------------- CONTAR CUENTAS ---------------------------------------
        
        static int ContarCuentasCliente(Banco banco, string dniCliente){
        	int contar = 0;
        	if(banco.CantidadCuentas() > 0){
        		foreach (Cuenta c in banco.ListarCuentas()){
        			if(c.DniTitular == dniCliente){
        				contar++;
        			}
        		}
        		return contar;
        	}else{
        		return 0;
        	}
        }

       
		//--------------------- MOSTRAR LISTA DE CLIENTES ---------------------------------

        static void ListarClientesConVariasCuentas(Banco banco)
        {
			Console.Clear();           
        	Console.WriteLine("=== CLIENTES CON VARIAS CUENTAS ===");
            List<Cliente> clientes = banco.ListarClientes();
            int hay_clientes = 0;

            foreach (Cliente c in clientes)
            {
                int cantidad = 0;
                List<Cuenta> cuentas = banco.ListarCuentas();

                //if (cantidad > 1)
                if(ContarCuentasCliente(banco, c.Dni) > 1)
                {
                	hay_clientes = hay_clientes +1;
                    Console.WriteLine(c.Apellido + ", " + c.Nombre + " - DNI: " + c.Dni);
                    Console.WriteLine("Tiene " + cantidad + " cuentas.");
                    //mostrar listado de cuentas que tiene el cliente con saldo
                    foreach (Cuenta cuenta in cuentas)
	                {
	                    if (cuenta.DniTitular == c.Dni)
	                    	Console.WriteLine("Nro: {0} - Saldo: {1}",cuenta.NroCuenta, cuenta.Saldo);
	                }
                    Console.WriteLine("-----------------------------");
                }
            }
            
            if(hay_clientes == 0){
            	Console.WriteLine("No hay clientes con mas de una cuenta.");
            }
        }

       
		//----------------------- REALIZAR EXTRACCIONES -----------------------------------
		
        static void RealizarExtraccion(Banco banco)
        {
            Console.WriteLine("=== EXTRACCIÓN ===");
            
            //validar dato
            bool nroOrigenValido = false;
            int nroCtaOrigen = 0;
            do{
            	Console.Write("Número de cuenta: ");
            	string nroOrigenStr = Console.ReadLine();
            	try{
            		//verificar numero de cuenta
					nroCtaOrigen = ValidarNroCta(nroOrigenStr);
            	}catch(FormatException ex){
            		Console.WriteLine(ex.Message);
            	}catch(ArgumentOutOfRangeException ex){
            		Console.WriteLine(ex.Message);
            	}
				nroOrigenValido = true;
            }while(!nroOrigenValido);
            //buscar cuenta
            Cuenta cuenta = BuscarCuenta(banco, nroCtaOrigen);

            if (cuenta != null)
            {
                Console.Write("Monto a extraer: ");
                string monto = Console.ReadLine();
                double saldoValor = 0;
                
                try{
                	saldoValor = ValidarSaldo(monto);
                }catch(FormatException ex){
	            	Console.WriteLine(ex.Message);
	            }catch(ArgumentOutOfRangeException ex){
	            	Console.WriteLine(ex.Message);
	            }catch(Exception ex){
	            	Console.WriteLine(ex.Message);
	            }
                
                if (saldoValor <= cuenta.Saldo)
                {
                    cuenta.Extraer(saldoValor);
                    Console.WriteLine("Extracción realizada. Saldo actual: " + cuenta.Saldo);
                }
                else
                {
                    Console.WriteLine("Saldo insuficiente.");
                }
            }
            else
            {
                Console.WriteLine("Cuenta no encontrada.");
            }
        }

      
		//------------------------ DEPOSITAR DINERO ---------------------------------------
		
		
        static void DepositarDinero(Banco banco)
        {
            Console.WriteLine("=== DEPÓSITO ===");
            
            //validar dato
            bool nroOrigenValido = false;
            int nroCtaOrigen = 0;
            do{
            	Console.Write("Número de cuenta: ");
            	string nroOrigenStr = Console.ReadLine();
            	try{
            		//verificar numero de cuenta
					nroCtaOrigen = ValidarNroCta(nroOrigenStr);
            	}catch(FormatException ex){
            		Console.WriteLine(ex.Message);
            	}catch(ArgumentOutOfRangeException ex){
            		Console.WriteLine(ex.Message);
            	}
				nroOrigenValido = true;
            }while(!nroOrigenValido);
            //buscar cuenta
            Cuenta cuenta = BuscarCuenta(banco, nroCtaOrigen);

            if (cuenta != null)
            {
                bool montoValido = false;
                double montoDouble = 0;
                do{
                	try{
                		Console.Write("Monto a depositar: ");
	                	string monto = Console.ReadLine();
	                	montoDouble = ValidarSaldo(monto);
	                	montoValido = true;
                	}catch(FormatException ex){
		            	Console.WriteLine(ex.Message);
		            }catch(ArgumentOutOfRangeException ex){
		            	Console.WriteLine(ex.Message);
		            }catch(Exception ex){
		            	Console.WriteLine(ex.Message);
		            }
                }while(!montoValido);
                cuenta.Depositar(montoDouble);
                Console.WriteLine("Depósito realizado. Saldo actual: " + cuenta.Saldo);
            }
            else
            {
                Console.WriteLine("Cuenta no encontrada.");
            }
        }

       
		//------------------------ HACER TRANSFERENCIAS -----------------------------------

        static void TransferirDinero(Banco banco)
        {
            Console.WriteLine("=== TRANSFERENCIA ===");
            bool nroOrigenValido = false;
            int nroCtaOrigen = 0;
            do{
            	Console.Write("Cuenta origen: ");
            	string nroOrigenStr = Console.ReadLine();
            	try{
            		//verificar numero de cuenta
					nroCtaOrigen = ValidarNroCta(nroOrigenStr);
            	}catch(FormatException ex){
            		Console.WriteLine(ex.Message);
            	}catch(ArgumentOutOfRangeException ex){
            		Console.WriteLine(ex.Message);
            	}
				nroOrigenValido = true;
								
            }while(!nroOrigenValido);
            //buscar cuenta
            Cuenta origen = BuscarCuenta(banco, nroCtaOrigen);

            bool nroDestinoValido = false;
            int nroCtaDestino = 0;
            do{
            	Console.Write("Cuenta destino: ");
            	string nroDestinoStr = Console.ReadLine();
            	try{
            		nroCtaDestino = ValidarNroCta(nroDestinoStr);
            	}catch(FormatException ex){
            		Console.WriteLine(ex.Message);
            	}catch(ArgumentOutOfRangeException ex){
            		Console.WriteLine(ex.Message);
            	}catch(Exception ex){
            		Console.WriteLine(ex.Message);
            	}
            	nroDestinoValido = true;
            }while(!nroDestinoValido);
            //buscar segunda cuenta
            Cuenta destino = BuscarCuenta(banco, nroCtaDestino);
	
            //si existen ambas cuentas realizar la operacion
            if (origen != null && destino != null)
            {
            	double montoATransferir = 0;
            	bool montoValido = false;
            	do{
            		Console.Write("Monto a transferir: ");
	                string monto = Console.ReadLine();
	                try{
	                	montoATransferir = ValidarSaldo(monto);
	                }catch(FormatException ex){
	                	Console.WriteLine(ex.Message);
	                }catch(ArgumentOutOfRangeException ex){
	                	Console.WriteLine(ex.Message);
	                }catch(Exception ex){
	                	Console.WriteLine(ex.Message);
	                }
	                montoValido = true;
            	}while(!montoValido);
                
                
                if (montoATransferir <= origen.Saldo)
                {
                    origen.Extraer(montoATransferir);
                    destino.Depositar(montoATransferir);
                    Console.WriteLine("Transferencia realizada correctamente.");
                }
                else
                {
                    Console.WriteLine("Saldo insuficiente en cuenta origen.");
                }
            }
            else
            {
                Console.WriteLine("Una o ambas cuentas no existen.");
            }
        }

        
		//---------------------- MOSTRAR LISTA DE CUENTAS ---------------------------------      

        static void ListarCuentas(Banco banco)
        {
            Console.WriteLine("=== LISTADO DE CUENTAS ===");
            List<Cuenta> cuentas = banco.ListarCuentas();

            foreach (Cuenta c in cuentas)
            {
                Console.WriteLine("Cuenta Nº " + c.NroCuenta + " - Titular DNI: " + c.DniTitular + " - Saldo: " + c.Saldo);
            }
        }

       
		//--------------------- MOSTRAR LISTA DE CLIENTES ---------------------------------

        static void ListarClientes(Banco banco)
        {
            Console.WriteLine("=== LISTADO DE CLIENTES ===");
            List<Cliente> clientes = banco.ListarClientes();

            foreach (Cliente c in clientes)
            {
                Console.WriteLine(c.Apellido + ", " + c.Nombre + " - DNI: " + c.Dni);
            }
        }
        
        
		public static void Main(string[] args)
		{
			
			//inicializar banco y cargarle los datos básico de nombre.
			Banco banco;
			banco = new Banco("Comisión");
			
			//Armado de menu
			
			//variable que ingresa el usuario
			//la inicialice en -1 porque si la dejaba en cero, salia directamente y no volvia al menu.
			int opcion = -1;
			do{
				Console.WriteLine("inicia menu");
	            Console.WriteLine("............. MENU PRINCIPAL .............");
	            Console.WriteLine("1 - Agregar cuenta");
	            Console.WriteLine("2 - Eliminar cuenta");
	            Console.WriteLine("3 - Listar clientes con más de una cuenta");
	            Console.WriteLine("4 - Realizar extracción");
	            Console.WriteLine("5 - Depositar dinero");
	            Console.WriteLine("6 - Transferir dinero entre cuentas");
	            Console.WriteLine("7 - Listar cuentas");
	            Console.WriteLine("8 - Listar clientes");
	            Console.WriteLine("0 - Salir");
	            Console.Write("Ingrese una opción: ");
	            
	            //verificamos si existe errores en el valor ingresado con exceptions
	            try{
					//obtener dato ingresado por el usuario
	            	string elegido = Console.ReadLine();
	            	
	            	if(elegido.Length == 1 && elegido[0] >= '0' && elegido[0] <= '8'){
	            		opcion = Convert.ToInt32(elegido);
	            	}else{
	            		//creo una exception si los valores no son ningunos de los esperados
	            		string mensaje_error = string.Format("El valor seleccionado: {0}. No es correcto. Elija una opción entre 0 y 8", elegido);
	            		throw new FormatException(mensaje_error);
	            	}
				}catch(FormatException ex){
	            	//limpio la consola 
	            	Console.Clear();
					//aca para verificar que sea un int
					Console.WriteLine("Error: {0}",ex.Message);
					
				}catch(Exception){
					Console.Write("Hubo un error");
				}
	            
	            //despues de pasar la validación seguimos con el switch case
	            //pasar la variable elegida a un switch 
	            //------------------DISTINTAS OPCIONES----------------------------
	            switch(opcion){
		            case 1:
		           		AgregarCuenta(banco);
		            	break;
		            case 2:
		           		EliminarCuenta(banco);
		            	break;
		            case 3:
		           		ListarClientesConVariasCuentas(banco);
		            	break;
		            case 4:
		            	RealizarExtraccion(banco);
		            	break;
		            case 5:
		            	DepositarDinero(banco);
		            	break;
		            case 6:
		            	TransferirDinero(banco);
		            	break;
		            case 7:
		            	ListarCuentas(banco);
		            	break;
		            case 8:
		            	ListarClientes(banco);
		            	break;
		            case 0:
		            	Console.WriteLine("Salir.");
		            	break;
		            default:
		                Console.WriteLine("Opción inválida. Intente nuevamente.");
		                break;
	            }
	            	
			}while(
				opcion != 0
			);
			
			Console.WriteLine("Hello World!");
			
			// TODO: Implement Functionality Here
			
			Console.Write("Press any key to continue . . . ");
			Console.ReadKey(true);
		}
	}
}
